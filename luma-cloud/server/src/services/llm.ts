...
type Provider = 'ollama' | 'openai' | 'anthropic' | 'mistral' | 'openrouter' | 'azure-openai' | 'gemini';
...
async function chatOllama(model: string, messages: { role: string; content: string }[]) {
...
async function chatOpenAI(endpoint: string, apiKey: string, model: string, messages: { role: string; content: string }[]) {
  ...
  if (!resp.ok) throw new Error(`OpenAI-like error: ${resp.status}`);
  const json = (await resp.json()) as any;
